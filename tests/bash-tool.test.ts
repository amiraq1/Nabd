...
process.env.TERMUX_BASH_PATH = 'bash';
...
import type { SecurityContext } from '../src/core/security/SecurityContext.ts';

const mockSecurityContext: SecurityContext = {
  role: 'ROOT_AGENT',
  permissions: ['safe', 'filesystem', 'network', 'system', 'dangerous'],
...
async function drainSession(
  ...
  for await (const event of session.stream()) {
    ...
    if (event.type === 'StdoutChunk') {
      ...
      for (const chunk of (event as any).chunks) {
        chunks.push({ chunk, isStderr: false });
      }
    } else if (event.type === 'StderrBatch') {
      for (const chunk of (event as any).chunks) {
        chunks.push({ chunk, isStderr: true });
      }
    }
  }
...
describe('executeBashTool (v2)', () => {
  ...
  it('returns a terminal error session for an empty command', async () => {
    const session = await executeBashTool.execute({ command: '' }, { security: mockSecurityContext });
    ...
    assert.ok(errorEvent && errorEvent.type === 'Failed');
  });
});
...
describe('toolEngine.execute(execute_bash)', () => {
  ...
  it('returns a ToolExecutionResult when streamV3 is not requested', async () => {
    ...
    // result is now a ToolExecutionResult
    assert.ok(result, 'expected a ToolExecutionResult');
  ...
  it('throws when the tool name is unknown', async () => {
    await assert.rejects(
      () => toolEngine.execute('not_a_real_tool', {}, { security: mockSecurityContext }),
      /Unknown tool/,
    );
  });
});
