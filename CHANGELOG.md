...
### Security
- The git status header subprocess forces `core.fsmonitor=false` via `-c`, runs status with `--no-optional-locks` and `--ignore-submodules=all`, and inspects effective repository configuration before running status, skipping it entirely if any `filter.*.clean` or `filter.*.process` command driver is defined. This prevents repository-local `.git/config` from executing arbitrary commands when opening an untrusted repository.
