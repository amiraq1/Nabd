...
**عدّ الرموز تخمين يُعاير.** لا مُرمِّز داخل البرنامج لكل النماذج؛ التقدير
الافتراضي قاعدة تُميّز الأحرف غير اللاتينية وتميل للمبالغة، تُعاير من
`input_tokens` الحقيقية إن وصلت من المزوّد. توقّع خطأً بحدود ١٠–٢٠٪ حتى
تُعايَر. للنماذج التي يتوفر لها جدول الدمج (BPE) يُستخدم مُرمِّز Go خالص
بدون CGO أو تبعيات ثقيلة. النموذج غير المعروف يسقط إلى القاعدة الاستدلالية
— يتدهور ولا يفشل. يُعرض الخطأ الجاري في `/ctx` (`· cal err +N%` أو
`· cal uncalibrated`).

**قراءة الملفات محدودة الحجم تكيفيًّا.** `read_file` يقصّ عند سقف يُحسب لكل
دورة من سقف TPM المزوّد والميزانية المتبقية من السياق — يقرأ أكثر حين يكون
السياق فارغًا وأقل حين يمتلئ. يُعاد حسابه كل دورة ويُخطر المستخدم عند تغيّره
مرة واحدة. إن وُضع `NABD_MAX_READ` صراحةً يُعطّل التكيّف ويُثبّت السقف عند
تلك القيمة (السلوك السابق). القصّ دائمًا على حدّ سطر، والنتيجة تخبر النموذج
بالنطاق وكيف يتابع بـ `offset`. القيم خارج [512, 1MiB] تُهمَل. إن تجاوز سطرٌ
وحده السقف، يُعرض مقتطعًا ويقفز `next_offset` إلى ما بعده — بقية ذلك السطر غير
قابلة للقراءة بهذه الأداة.
...
`internal/token` عدّ الرموز: مُرمِّز BPE خالص (بدون CGO) مع سقوط احتياطي إلى القاعدة الاستدلالية.
