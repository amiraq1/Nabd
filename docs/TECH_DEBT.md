...
| STREAM_REDACT_PARENT_REUSE_REWIND_UNVERIFIED | The on-disk event `Seq` and the loop's in-memory `seq` are independent spacings. `newStreamRedactSink` (`cmd/ag/stream_redact.go`) drops a `text_delta` whose redacted output is empty, holds its sequence number, and reuses that number for the boundary flush that carries the held bytes, rewriting `Parent` only where it pointed at the dropped delta. The invariant enforced instead of reconciling the two spacings is that every `Parent` written to the journal resolves to an event present in the journal. Coverage (`cmd/ag/stream_redact_test.go`) exercises the parent chain, boundary flushes, and a live rewind, but one state is untested: a rewind to a branch point whose `Parent` is a sequence that was dropped and then reused by a later flush. | A change to the reuse rule could leave a `Parent` pointing at a number that was dropped and never reused; `agent.Live` stops at such a `Parent` and would silently truncate a resumed branch. Guard: add a regression test that rewinds to exactly that point before the reuse rule is next touched. |
