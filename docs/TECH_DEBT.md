...
`TestRewindToReusedParentKeepsChainResolvable` (`cmd/ag/stream_redact_test.go`) now runs instead of skipping, so the one state this entry named is covered by a test that executes in CI. The test proves its own precondition — a `text_delta` withheld because its redacted output was empty, its sequence number reused by the boundary flush, and a journal event whose `Parent` is that reused number — then rewinds, continues into a second run, and asserts that seqs are unique and strictly increasing, that every `Parent` resolves to an event present in the journal, that `agent.Live` returns the whole chain through the last event, and that replay and the in-memory history agree on the branch point.

The committed skip recorded a harness defect, not a reuse-rule defect: `multiChunkProvider` was one turn short for the sequence the test performs, so the fix is a third scripted turn in the test. Guard evidence: making the boundary flush take a fresh number instead of the held-open dropped one (`Seq: s.lastDropped + 1`) fails the test with `duplicate seq 11` — the flushed delta and the next event share a sequence number, and the live chain truncates.

## ALLOWED_NOTICE_TEXT_PROVENANCE — RESOLVED by structured, bounded notice rendering

`renderNotice` in `internal/agent/messages.go` is now the only translation from an allowed notice event to the model-facing line. A structured `NoticeData` payload is authoritative and the human `Event.Text` is never consulted beside it, so an emitter can no longer smuggle prose into the model's context; a payload that does not match its `NoticeCategory` is dropped rather than repaired (`NoticeData.validate`); text-only events, which is how journals written before the payload decode, still render — sanitized and bounded — so replaying an old session is unchanged in kind. Every line passes `provider.SanitizeBody` for credential redaction, has every control rune collapsed to a space so one notice is always one logical line, cannot open with the `«notice»` frame marker, and is capped at `maxNoticeBytes` with an explicit `…[notice truncated]` marker. The emitters carry the payload: `Loop.NoteUndo` takes the reverted and failed path lists, and both `checkLoop` thresholds carry a `LoopLimitNotice`. The provenance half is held by `TestAllowedNoticeEmitsCarryStructuredPayload`, which walks every production source in the module with `go/parser`, fails if an allowed category is ever constructed with raw `Text` and no payload, and enforces that every allowed category has at least one production emit site (`sitesPerCategory[cat] > 0`). Negative test `TestAllowedNoticeGuardDetectsCategoryWithoutEmitter` proves that orphan categories with zero emitters are caught and fail the suite. Tool execution refusals are reported exclusively through failing `ToolEnd` results (the established architectural contract where every tool use receives a corresponding result block), so the un-emitted `NoticeCategoryPermissionDenied` category is eliminated to avoid dead schema, vacuous guard passes, and duplicate model-facing notices.
...
doChat runs ui.Chat, doChatWithFeed runs ui.Feed, and doReplay runs
ui.NewReplay. The -feed flag defaults to true (flag.Bool("feed", true, ...)),
so the default interactive path is Feed, while --feed=false selects Chat as a
rollback mechanism.
...
## SESSION_PATH_COLLISION (Finding 1) — RESOLVED by exclusive random naming

New sessions use `sessionPathAt` → `newSessionName`: `<timestamp>-r<RANDOM>.jsonl`
when the system random source is available, with PID+counter only as a
fallback. `newSessionJournalWithOptions` then creates the candidate with
`NewJSONLExclusiveWithOptions` and retries on `os.ErrExist`.

The previous PID-namespace collision concern is closed for new sessions.
`NewJSONL` remains append-oriented by design for `--continue`; callers must
not use it to allocate a new session.
