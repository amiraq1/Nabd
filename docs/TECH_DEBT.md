...
| BASH_POST_REAP_SWEEP_UNGUARDED | The post-reap `killGroup` removal in `bash.go` (PR #139, fix #3) is guaranteed only by a code comment, not by a test. A future edit re-adding `killGroup(pgid)` after `Wait` would silently reintroduce the recycled-PGID signal bug. | Add a test asserting that `killGroup` is not called on the clean-exit path, or that a source scan of `bash.go` shows no `killGroup` call outside the timeout/cancellation branches. See `docs/reports/pr139_audit.md` fix #3. |
