...
| Stream redactor never splits a complete match across emitted and held chunks | REDUCED | Cut points that straddle a recognized secret pattern or configured exact key are clamped back to the match boundary, ensuring complete credentials are not split into an unredacted emitted prefix and held suffix while keeping buffer growth bounded. Evidence: `TestStreamCutNeverSplitsCompleteMatch`, `TestStreamCutNeverSplitsExactKey`, `TestStreamPendingBoundedOnLongBearerChain` |
