...
| Provider `providers.json` baseURL and `NABD_BASE_URL` override are validated at load time: plaintext http and non-public addresses are refused by default | GUARANTEED | `endpoint.CheckBaseURL` runs during `ParseProvidersData` and `BuildStandaloneProviderWithRegistry`, rejecting any declared or overridden `baseURL` that uses a non-https scheme or resolves to a loopback, RFC 1918, link-local, CGNAT, ULA, cloud-metadata literal IP, or `.internal`/`.local`/`localhost`-suffixed hostname. The active policy is read from `NABD_ENDPOINT_POLICY` (default `strict`); `loopback` relaxes the host check to allow local runtimes with HTTPS; `open` disables both layers. Evidence: `TestStrictRefusesPlaintextHTTP`, `TestStrictRefusesCloudMetadataLiteral`, `TestStrictRefusesCGNATLiteral`, `TestStrictRefusesIPv6ULA`, `TestLoopbackStillRefusesHTTP`, `TestOpenLevelDisablesBothLayers` |
| Provider dial-time endpoint guard evaluates the socket address via Dialer.Control immediately before connect, eliminating the TOCTOU rebinding window | GUARANTEED | `Policy.Dialer` configures `net.Dialer.Control` to inspect the literal socket IP:port right before `connect(2)`. No secondary DNS lookup is performed, closing the DNS-rebinding window with zero TOCTOU: a hostname resolving to a private, loopback, link-local, CGNAT, ULA, or metadata IP is aborted before any packet is transmitted. Evidence: `TestControlRefusesRebindingViaResolver`, `TestControlRefusesRebindingToPrivateAddress`, `TestControlRefusesLoopbackAddress`, `TestControlRefusesCloudMetadata`, `TestControlRefusesCGNATAddress`, `TestControlRefusesIPv6ULAAddress`, `TestControlPassesThroughPublicAddress` |
| Endpoint policy is wired into every provider constructor and catalog probe via a controlled HTTP client | GUARANTEED | `endpoint.Client` is called in `BuildRouteProviderWithRegistry`, `BuildStandaloneProviderWithRegistry`, `cmd/ag`'s `runModelsCommand`, and `providercmd.FetchModels`; both inference routes and `/models` catalog probes receive an `*http.Client` whose transport uses `Policy.Dialer`. No inference or catalog request can bypass the connect-time guard. Evidence: `TestControlRefusesRebindingViaResolver`, `TestClientConstructors`, `TestCustomEndpointIsAcceptedByDesign` |
