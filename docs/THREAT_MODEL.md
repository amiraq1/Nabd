...
| Release artifacts are signed | GUARANTEED | `.goreleaser.yaml` signs every binary artifact with `cosign sign-blob` and stores the certificate; the build is pinned to a known Go toolchain. The signed manifest is published as `checksums.txt` alongside `checksums.txt.sig` (the cosign signature) and `checksums.txt.pem` (the certificate), so verification needs only the release assets. Evidence: `TestReleasePipelineContracts` |
