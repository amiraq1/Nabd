...
| Security gate scripts are mechanically checked | GUARANTEED | `scripts/check-pr-security-checklist.sh` enforces Phase 1 of the PR template and Phase 2 regression-test diff cross-checks with narrow N/A explanations for non-executable changes; `scripts/check-threat-model-tests.sh` verifies every backtick-quoted test citation in this document resolves to a real `Test*` function. Evidence: `TestPRChecklistGateScopesThreatModelClaim`, `TestPRChecklistGateSecurityPathsAreRealBoundaries` |
