...
| Stream redactor hold-back state transitions preserve PEM boundary markers | REDUCED | Over-long token runs being swallowed across chunks terminate before any PEM block boundary, and PEM blocks are evaluated after swallow transitions so boundary markers remain intact. Evidence: `TestStreamSwallowPreservesPEMBoundary` |
