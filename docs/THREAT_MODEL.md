...
| Plain headless stdout and interactive assistant text are not redacted | NOT PROVIDED | Plain headless stdout (`finalAssistantText` over agent history) and assistant text in the interactive feed are not redacted by design: they are the user's requested output. Journal, `--json`, copy and `--export --redact` are redacted. Evidence: `TestHeadlessPlainStdoutNotRedactedByDesign`, `TestHeadlessJSONRedactsSplitSecret`, `TestCopyRedactsRecognizedCredentials`, `TestExportLeavesSourceUntouched` |
