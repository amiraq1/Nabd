...
| Resource exhaustion from approved bash | REDUCED | `NABD_BASH_RESOURCES=limit` is opt-in and, when the Linux helper is available, applies inherited limits of 600 CPU seconds, 4 GiB address space, 256 processes, and 1024 open files; it fails closed if the helper or platform support is unavailable. Disk usage and kernel-wide per-user quotas remain outside this boundary. Evidence: `TestLandlockAppliesResourceLimits`, `TestParseResourceMode`, `TestSelectResourceLimitsFailsClosed` |
