...
| Unresolved mutation intents are visible on continuation without automatic replay | GUARANTEED | When the active branch contains an `edit_intent` without a matching `edit_record` or `edit_abort`, `--continue` emits a display-only recovery notice stating the count and instructing the operator to verify the working tree. It never re-executes the mutation and ignores abandoned branches. Evidence: `TestUnresolvedMutationIntentRequiresExplicitRecovery`, `TestCommittedOrAbortedMutationIsNotUnresolved`, `TestUnresolvedMutationIgnoresAbandonedBranch` |
