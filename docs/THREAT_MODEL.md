...
| File reads open through descriptor-relative operations and refuse symlinks and non-regular files | GUARANTEED | `internal/safefs.OpenRead` walks the relative path from a root descriptor with `O_NOFOLLOW` and validates the target from the opened descriptor with `Fstat`. Platform containment is unconditional (Unix-only by construction). Evidence: `TestOpenReadRefusesFinalSymlink`, `TestOpenReadRefusesIntermediateSymlink`, `TestOpenReadRefusesFIFO`, `TestOpenReadRefusesDirectory`, `TestCaptureFromRootRejectsFinalSymlink`, `TestCaptureFromRootRejectsIntermediateSymlink`, `TestUnixOnlyPlatformBoundary` |
| File mutations publish relative to the parent descriptor and re-prove the target at the syscall | GUARANTEED | `write_file`, `edit_file`, and `/undo` derive one relative path and never open, stat, or rename a project file by absolute path; the absolute path is reporting metadata only. Evidence: `TestWritePathFromRootRejectsTraversal`, `TestWriteFileAtomicRefusesIntermediateSymlink`, `TestWriteFileAtomicReplacesFinalSymlinkWithoutFollowing`, `TestWriteFromRootDoesNotUseAbsoluteMetadataAsAuthority`, `TestCommitRejectsSymlinkWithoutCapturingOutsideContent`, `TestReadSourceFromRootUsesRelativeAuthority`, `TestRemoveFromRootUsesRelativeAuthority`, `TestRemoveFileRemovesFinalSymlinkNotReferent`, `TestUndoRefusesModifiedAfterAgentWrite`. Supplementary structural guards (AST and platform boundary): `TestWriteCommitDelegatesToAdapters`, `TestWriteCommitUnixAdapterIsDescriptorOnly`, and `TestUnixOnlyPlatformBoundary` |
...
| Path opening after `Resolve` outside the descriptor layer | REDUCED | `bash` is not contained; `snap.Restore` / `snap.RestoreAt` are a path-based publish path, now test-only (`docs/TECH_DEBT.md`); configuration reading still traverses parent-directory symlinks. `!unix` compatibility paths have been permanently deleted, enforced by `TestUnixOnlyPlatformBoundary`. Residual: a same-uid attacker through those paths |
...
| Provider registry auth file enforces private 0600 permissions, owner check, and redaction in errors (unix-only-by-construction) | GUARANTEED | Enforced via kernel descriptor validation on Unix; non-Unix platforms and permissive stubs are rejected at build time. Evidence: `TestAuthFileRejectsOpenPermissions`, `TestAuthKeysAreRedactedInErrors`, `TestUnixOnlyPlatformBoundary` |
...
| Endpoint policy is wired into every provider constructor and catalog probe via a controlled HTTP client with proxy validation | GUARANTEED | Every provider constructor (`NewOpenAIDialect`, `NewAnthropicDialect`) defaults to `endpoint.Client(0)` with a guarded transport, and both route/standalone builders and `/models` probes use this client. When an environment proxy (`HTTPS_PROXY` / `HTTP_PROXY`) is configured, the proxy URL is validated against the endpoint policy at request time; under PolicyStrict, plaintext http or private proxies are refused before dialing. Note: when a proxy is configured, the connect-time dial check applies to the proxy server address rather than the origin server, which receives requests via HTTP CONNECT. Evidence: `TestControlRefusesRebindingViaResolver`, `TestConstructorsDefaultToGuardedClient`, `TestTransportProxyValidation`, `TestCustomEndpointIsAcceptedByDesign` |
...
`!unix` compatibility paths and Windows stubs are permanently excised; descriptor-relative operations and kernel-enforced ownership checks are unconditional across all supported platforms (enforced by `TestUnixOnlyPlatformBoundary`). `snap.Restore` and
