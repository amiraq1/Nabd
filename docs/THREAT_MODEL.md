...
| Agent notices forwarded to the model context are restricted to an enumerated allowlist that defaults to deny | GUARANTEED | Only notices that change world-state (such as /undo results) or restrict required capabilities (permission denial, loop limits) reach the provider message projection; monitoring, calibration, and display notices never enter the model conversation. Forwarded notices are explicitly tagged («notice») as agent/system events rather than raw user text. The tag is a semantic signal that does not technically prevent model compliance with allowed notices; the guarantee is narrowing injectable content, not immunizing the model against permitted notices. Evidence: `TestCalibrationNoticeNeverReachesTheModel` |
...
| Provider `providers.json` baseURL is validated at load time: plaintext http and non-public addresses are refused by default | GUARANTEED | `endpoint.CheckBaseURL` runs during `ParseProvidersData` and rejects any declared `options.baseURL` that uses a non-https scheme or resolves to a loopback, RFC 1918, link-local, cloud-metadata literal IP, or `.internal`/`.local`/`localhost`-suffixed hostname. The active policy is read from `NABD_ENDPOINT_POLICY` (default `strict`); `loopback` relaxes the host check to allow local runtimes with HTTPS; `open` disables both layers. Evidence: `TestStrictRefusesPlaintextHTTP`, `TestStrictRefusesCloudMetadataLiteral`, `TestLoopbackStillRefusesHTTP`, `TestOpenLevelDisablesBothLayers` |
| Provider dial-time endpoint guard refuses private and metadata addresses after DNS resolution, closing the rebinding path | GUARANTEED | `endpoint.DialControl` wraps `net.Dialer.DialContext` and, after DNS resolution, refuses any resolved IP in loopback, RFC 1918, link-local, or cloud-metadata ranges. This connect-time check closes the DNS-rebinding path: a public hostname in `providers.json` that resolves to a private IP at connection time is refused without completing the dial. The guard is active under PolicyStrict (default); PolicyLoopback and PolicyOpen pass through unconditionally. Evidence: `TestControlRefusesRebindingToPrivateAddress`, `TestControlRefusesLoopbackAddress`, `TestControlRefusesCloudMetadata`, `TestControlPassesThroughPublicAddress` |
| Endpoint policy is wired into every provider constructor via a controlled HTTP client | GUARANTEED | `controlledHTTPClient` is called in `BuildRouteProviderWithRegistry` and `BuildStandaloneProviderWithRegistry`; both the router and standalone paths receive an `*http.Client` whose `Transport.DialContext` is `endpoint.DialControl`. No provider constructor can bypass the guard by ignoring a policy flag, because the guard lives in the transport layer, not in any caller-optional code path. Evidence: `TestControlRefusesRebindingToPrivateAddress`, `TestCustomEndpointIsAcceptedByDesign` |
| Loopback runtimes require HTTPS even under PolicyLoopback | GUARANTEED | `endpoint.CheckBaseURL` checks the scheme before the host when `NABD_ENDPOINT_POLICY=loopback`: http is still refused; only the host-range check is relaxed. A local ollama server must use HTTPS. Evidence: `TestLoopbackStillRefusesHTTP`, `TestLoopbackAllowsLoopbackAndPrivate` |
| `NABD_ENDPOINT_POLICY=open` disables both the load-time URL check and the connect-time dial guard | REDUCED | When an operator sets `NABD_ENDPOINT_POLICY=open`, `CheckBaseURL` returns nil for any URL and `DialControl` returns the base dialer unchanged. This is the override for air-gapped or development environments that cannot use HTTPS. Whoever can set this variable can direct all connections to an arbitrary endpoint including plaintext and private addresses. The remaining controls are the 0600 permission and owner check on `providers.json` and `auth.json`. Evidence: `TestOpenLevelDisablesBothLayers`, `TestControlIsNoOpUnderOpenPolicy` |
...
| Bash filesystem reach after approval | REDUCED | On Linux kernels with Landlock support, the child is restricted before `sh` starts: the project root and per-invocation HOME/TMP directories are writable, runtime directories are read/execute-only, and paths outside those allowlists are denied. `NABD_BASH_SANDBOX=auto` (the default) retains the documented user-authority fallback on older kernels and non-Linux platforms; `NABD_BASH_SANDBOX=on` fails closed instead of falling back, while `off` is an explicit operator override. This boundary does not restrict network access. Evidence: `TestLandlockRestrictsOutsideRoot`, `TestParseMode`, `TestUseHelperModes` |
| Network/resource exhaustion from approved bash | REDUCED | Network remains allowed by default for approved commands. `NABD_BASH_NETWORK=deny` uses Landlock ABI 4+ to deny TCP bind/connect and fails closed when that boundary is unavailable; UDP, raw sockets, and CPU/memory/process resource exhaustion remain outside this control. Evidence: `TestLandlockDeniesNetwork`, `TestParseNetworkMode`, `TestSelectNetworkDenyFailsClosed` |
| Resource exhaustion from approved bash | REDUCED | `NABD_BASH_RESOURCES=limit` is opt-in and, when the Linux helper is available, applies inherited limits of 600 CPU seconds, 4 GiB address space, 256 processes, 1024 open files, and 256 MiB per file; it fails closed if the helper or platform support is unavailable. Aggregate directory usage and kernel-wide per-user quotas remain outside this boundary. Evidence: `TestLandlockAppliesResourceLimits`, `TestParseResourceMode`, `TestSelectResourceLimitsFailsClosed` |
