...
| Shadow store | `<root>/.ag/shadow`, content-addressed `s256:` blobs containing full pre/post-edit content. `.ag` and `.ag/shadow` are tightened to 0700 and `.ag/.gitignore` is created once with the single rule `*` (an existing `.gitignore` is never read, appended to, or replaced), so the whole store stays invisible to git. |
...
| The project shadow store is invisible to git, and shielding it never touches an existing `.gitignore` | GUARANTEED | On first use the shield writes `<root>/.ag/.gitignore` exactly once, as the single rule `*`, atomically with mode 0600 inside the 0700 store directory, so `git status --porcelain` and `git add -A` neither show nor stage any `.ag/` path; an existing `.gitignore` (user content, or an older shield's `/shadow/` rule) is never read, appended to, or replaced, and `/undo` keeps restoring from the store. Evidence: `TestShieldGitignoreKeepsProjectStoreOutOfGit`, `TestShieldGitignoreNeverReplacesExistingFile`, `TestUndoStillWorksWithShieldedGitignore` |
