...
| Security gate scripts are mechanically checked | GUARANTEED | `scripts/check-pr-security-checklist.sh` enforces Phase 1 of the PR template and Phase 2 regression-test diff cross-checks with narrow N/A explanations for non-executable changes; `scripts/check-threat-model-tests.sh` verifies every backtick-quoted test citation in this document and in `docs/TECH_DEBT.md` resolves to a real `Test*` function (`NOTES.md` is deferred pending a historical-annotation convention; see `TECH_DEBT.md §NOTES_CITATION_GUARD_DEFERRED`). Evidence: `TestPRChecklistGateScopesThreatModelClaim`, `TestPRChecklistGateSecurityPathsAreRealBoundaries`, `TestPRChecklistGateNoTestExemption`, `TestCitationGuardPassesOnCleanRepository`, `TestCitationGuardTechDebtIncluded` |
