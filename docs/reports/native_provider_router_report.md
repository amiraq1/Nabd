...
- **الصيغة القانونية:**
  ...
  ```
- **معرّف المزود حرّ:** لم تعد هناك قائمة مغلقة. أي اسم يطابق `[a-z0-9-_]` وبطوله ≤ 32 بايت يُقبل عند التحليل. هل المزود *مضبوط* فعلًا يُقرَّر لاحقًا وقت بناء المسار من السجلّ (الكتالوغ المضمَّن ثم `~/.ag/providers.json` ثم `~/.ag/auth.json`)، ورسالة الخطأ تسمّي المزوّد المطلوب والمضبوطين فعلًا وملف `providers.json` الذي يُضاف فيه.
...
أي مثال في هذه الوثيقة أو في غيرها يجب أن يستخدم مزودًا مضبوطًا فعلًا: أحد مزوّدي الكتالوغ المضمَّن (`anthropic`, `groq`, `openrouter`, `nvidia`) أو مزودًا يعرّفه المستخدم في `~/.ag/providers.json`. مثالٌ يستخدم مزودًا غير مضبوط يُرفض عند بناء المسار بالخطأ `route provider ... is not configured` مع ذكر ملف `providers.json`. أضف المزوّد الجديد بمفتاحه في `~/.ag/auth.json` عبر `/connect`. وكذلك أسماء النماذج: القائمة الحيّة عند المزود (`/v1/models`) وطلبٌ حقيقي هما المرجع الوحيد، لأن ورود اسم في كتالوج لا يعني أن مفتاحك مُصرَّح له به.
