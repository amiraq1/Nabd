...
| 3 | **bash-no-post-reap-sweep** | `internal/tools/bash.go:141` | Before: `killGroup(pgid)` was called unconditionally after `Wait`, including after clean exit. A recycled process-group ID could signal unrelated processes. After: the post-reap `killGroup` is removed; only timeout/cancellation paths signal the group before `Wait`. | `UNGUARDED` — no test asserts the absence of a post-reap sweep or the kill-only-before-wait invariant. The THREAT_MODEL row cites the code comment as evidence, not a test. | None | Yes |
...
|---|---|---|
| **#3 — bash-no-post-reap-sweep** | "A bash cleanup signal cannot target a recycled process-group ID after the leader has been reaped." The THREAT_MODEL row for this guarantee cites the code comment (`killGroup` runs only on timeout/cancellation) as evidence, not a test. No test asserts the absence of a post-reap `killGroup` call, nor that clean-exit does not signal the group. A future edit adding `killGroup(pgid)` after `Wait` would silently reintroduce the bug. | Medium — the window for a recycled PGID to be signaled is narrow but the consequence (killing unrelated processes) is severe. |
...
- **#3** → `docs/TECH_DEBT.md` row: `BASH_POST_REAP_SWEEP_UNGUARDED`
