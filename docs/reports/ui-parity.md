...
* التباعد (سابقاً): `doChat` و `headless.go` يستعملان `filepath.Base(cwd)`، بينما `doChatWithFeed` يستعمل `filepath.Base(journalPath)`.
* **حُلّ بالحذف لا بالتوحيد:** أُزيل `doChat` مع تقاعد Chat (ADR-0001)، فلم يبقَ إلا `doChatWithFeed` و`headless`، ولا يوجد مصدر ثانٍ للبانر يتباعد عنه.
...
> **تحديث 2026-09-22 — نُفِّذ.** كانت هذه الخارطة تصف ثلاث مراحل
> (`v1.6.0` → `v1.7.0` → `v1.8.0`). لم يُنشر `v1.6.0` قط (فشل التوقيع) ولم
> يشحن `v1.6.1` سوى إصلاح التوقيع، فلم يعرض أي إصدار ثنائي `--ui` أو
> `NABD_UI` قط. لذلك **طُويت مرحلتا v1.6.0 وv1.7.0 في هبوط واحد**، والجدول
> أدناه هو السلوك الفعلي المُنفَّذ الآن، لا خطة مستقبلية. مرحلة `v1.8.0`
> (حذف جذمور `--feed`) باقية في موعدها.

يُنفَّذ منطق التحقق في `cmd/ag/main.go` عبر `checkUISurface(uiFlag string, useFeed bool, provided map[string]bool) (string, error)` مع `rejectUIFlagWith` لرفض الأعلام مع الأوضاع غير التفاعلية:

| الحالة | المدخلات | السلوك الفعلي المُنفَّذ |
|---|---|---|
| **افتراضي** | لا أعلام ولا بيئة | `feed` |
| **علم قديم موجب** | `-feed` / `--feed=true` | `feed` + تحذير إهمال يسمّي `--ui` |
| **علم قديم سالب** | `--feed=false` | رفض (خروج 2) + إرشاد: السطح المتقاعد، استعمل `--ui=feed` |
| **علم جديد** | `--ui=feed` | `feed` نظيف |
| **علم جديد مهمل** | `--ui=chat` | رفض (خروج 2) + رسالة تشرح تقاعد Chat |
| **علم جديد مجهول** | `--ui=bogus` | رفض (خروج 2) |
| **تعارض أعلام** | `--ui=feed --feed=false` | رفض (خروج 2) |
| **علم وبيئة** | `--ui` مع `NABD_UI` | العلم يفوز دائماً |
| **بيئة مهملة** | `NABD_UI=bogus` أو `NABD_UI=chat` | تحذير + التراجع إلى `feed` (ليس خطأً حاجباً) |
| **علم مع وضع غير تفاعلي** | `--ui`/`--feed` مع `--replay` أو `-p` أو `--export` | رفض (خروج 2) + `set NABD_UI for a persistent preference` |

---

## ٣. قاعدة الحراسة المعمارية (AST-Based Archtest)

**مُفعَّلة الآن** (2026-09-22) في `internal/archtest/chat_removed_test.go`. الشيفرة أدناه هي المرجع التصميمي الأصلي؛ التنفيذ الفعلي يمسح شجرتَي `cmd/` و`internal/` ويطابق المعرّفات على **عُقد AST لا على النص**، فلا يكفيه تعليق ولا تكسره إعادة تسمية. قائمة المعرّفات المحظورة مطابقة:
