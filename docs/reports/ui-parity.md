...
* **إنفاذ تعاقدي:** `TestCopyUnavailableOutsideTermuxAndSSH` يثبّت البوابة؛ إزالة شرط SSH ستكسر هذا الاختبار، ولذلك اعتُمدت البوابة خياراً موثّقاً لا انحداراً.
* **الديون المرتبطة:**
  - **N4:** كتابة OSC 52 المتزامنة إلى `os.Stdout` ما زالت داخل دورة التحديث؛ تتطلب إعادة هيكلة إلى `tea.Cmd` لاحقاً.
  - **N2:** فشل الإنقاذ إلى ملف كان صامتاً؛ صار مُعلناً في سطر الحالة.
