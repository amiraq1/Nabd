...
// lineLayout caches the wrapping layout of a single logical line.
...
type lineWrapCache struct {
	width int
	lines map[int]*lineLayout
}
...
func slicesEqualRunes(a, b []rune) bool {
	if len(a) != len(b) {
		return false
	}
	if len(a) == 0 {
...
func commonRunesPrefix(a, b []rune) int {
	n := len(a)
	if len(b) < n {
		n = len(b)
	}
...
func wrapWithOffsets(runes []rune, width int) ([][]rune, []int) {
	wrapped := wrap(runes, width)
	offsets := make([]int, len(wrapped))
	if len(wrapped) > 0 {
		offsets[0] = 0
		for i := 1; i < len(wrapped); i++ {
			offsets[i] = offsets[i-1] + len(wrapped[i-1])
		}
	}
...
func New() Model {
	...
	m := Model{
		...
		layoutCache:          &lineWrapCache{lines: make(map[int]*lineLayout)},
...
func (m *Model) resetLayoutCache() {
	if m.layoutCache != nil {
		m.layoutCache.lines = make(map[int]*lineLayout)
	}
}
...
func (m *Model) SetWidth(w int) {
	...
	newWidth := inputWidth - reservedOuter - reservedInner
	if m.width != newWidth {
		m.resetLayoutCache()
	}
	m.width = newWidth
...
func (m Model) View() string {
	...
	for l, line := range m.value {
		wrappedLines := m.memoizedWrap(line, m.width, l)
...
func (m Model) memoizedWrap(runes []rune, width int, lineIdx ...int) [][]rune {
	if m.layoutCache == nil {
		return wrap(runes, width)
	}

	if m.layoutCache.width != width {
		m.layoutCache.width = width
		m.layoutCache.lines = make(map[int]*lineLayout)
	}
	...
	if len(lineIdx) > 0 {
		idx = lineIdx[0]
	} else if m.row < len(m.value) {
		idx = m.row
	}

	entry := m.layoutCache.lines[idx]
	if entry != nil && entry.width == width {
		if len(entry.runes) == len(runes) && (len(runes) == 0 || &entry.runes[0] == &runes[0] || slicesEqualRunes(entry.runes, runes)) {
			return entry.wrapped
		}

		prefixLen := commonRunesPrefix(entry.runes, runes)
		if prefixLen > 0 && len(entry.wrapped) > 0 && len(entry.offsets) == len(entry.wrapped) {
			editRow := len(entry.wrapped) - 1
			for i := 0; i < len(entry.offsets)-1; i++ {
				if entry.offsets[i+1] > prefixLen {
					editRow = i
					break
				}
			}
			...
			reuseRows := editRow - 1
			if reuseRows > 0 && reuseRows < len(entry.wrapped) && reuseRows < len(entry.offsets) {
				reuseOffset := entry.offsets[reuseRows]
				if reuseOffset <= len(runes) {
					remainderRunes := runes[reuseOffset:]
					remainderWrapped := wrap(remainderRunes, width)

					newWrapped := make([][]rune, 0, reuseRows+len(remainderWrapped))
					for i := 0; i < reuseRows; i++ {
						newWrapped = append(newWrapped, entry.wrapped[i])
					}
					newWrapped = append(newWrapped, remainderWrapped...)

					newOffsets := make([]int, len(newWrapped))
					for i := 1; i < len(newWrapped); i++ {
						newOffsets[i] = newOffsets[i-1] + len(newWrapped[i-1])
					}

					newEntry := &lineLayout{
						width:   width,
						runes:   cloneRunes(runes),
						wrapped: newWrapped,
						offsets: newOffsets,
					}
					m.layoutCache.lines[idx] = newEntry
					return newWrapped
				}
			}
		}
	...
	wrapped, offsets := wrapWithOffsets(runes, width)
	m.layoutCache.lines[idx] = &lineLayout{
		width:   width,
		runes:   cloneRunes(runes),
		wrapped: wrapped,
		offsets: offsets,
...
func (m Model) cursorLineNumber() int {
	...
	for i := 0; i < m.row; i++ {
		...
		// into.
		line += len(m.memoizedWrap(m.value[i], m.width, i))
	}
