...
func TestV2StrictRouterPreservesRouteOrder(t *testing.T) {
	...
	if got["NABD_ROUTES"] != want { t.Fatalf("routes=%q want %q", got["NABD_ROUTES"], want) }
}
...
func TestV2RejectsUnknownAndCommandSource(t *testing.T) {
	...
	for name, body := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := os.WriteFile(p, []byte(body), 0o600); err != nil { t.Fatal(err) }
			if _, err := ParseV2File(p); err == nil { t.Fatal("invalid v2 accepted") }
		})
	}
}
...
func TestV1AndV2TogetherAreFatal(t *testing.T) {
	...
	if err := os.WriteFile(v2, []byte(`{"version":2}`), 0o600); err != nil { t.Fatal(err) }
	t.Setenv(EnvVar, v1)
	t.Setenv(V2EnvVar, v2)
	if _, _, err := SelectedPath(); err == nil { t.Fatal("v1 and v2 accepted together") }
}
...
func TestV2DisablesImplicitEnvironmentFallback(t *testing.T) {
	...
	if got := Get("GROQ_API_KEY"); got != "" { t.Fatal("undeclared env credential leaked into v2") }
}
...
func TestV2CredentialFileAndEndpointPolicy(t *testing.T) {
	...
	got, err := ParseV2File(p)
	if err != nil { t.Fatal(err) }
	if got["OPENROUTER_API_KEY"] != "file-secret" { t.Fatal("credential file not loaded") }
	for _, raw := range []string{"http://example.com", "https://127.0.0.1/v1", "https://169.254.169.254/v1", "https://localhost/v1"} {
		if err := ValidateEndpointURL(raw); err == nil { t.Fatalf("unsafe URL accepted: %s", raw) }
	}
}
...
func TestV2ErrorsNeverContainCredentialValue(t *testing.T) {
	...
	p := filepath.Join(t.TempDir(), "config.v2.json")
	if err := os.WriteFile(p, []byte(`{"version":2,"provider":"router","routes":[],"credentials":{"groq":{"source":"env"}}}`), 0o600); err != nil { t.Fatal(err) }
	_, err := ParseV2File(p)
	if err == nil || strings.Contains(err.Error(), sentinel) { t.Fatalf("unsafe error: %v", err) }
}
