...
func AllSlashCommands() []SlashCommand { out := make([]SlashCommand, len(supportedSlashCommands)); copy(out, supportedSlashCommands); return out }
...
type ParsedSlashCommand struct { Command SlashCommand; RawCmd string; N int; HasN bool; Valid bool; Error string }
...
func FilterSlashCommands(query string) []SlashCommand {
	...
	if trimmed == "" || trimmed == "/" { all := AllSlashCommands(); if len(all) > 8 { return all[:8] }; return all }
	q := strings.ToLower(trimmed); qNoSlash := strings.TrimPrefix(q, "/")
	...
	for _, cmd := range supportedSlashCommands {
		...
		default:
			aliasMatch := false
			for _, a := range cmd.Aliases { al := strings.ToLower(a); if strings.HasPrefix(al, q) || strings.HasPrefix(strings.TrimPrefix(al, "/"), qNoSlash) { aliasMatch = true; break } }
	...
	out := make([]SlashCommand, 0, len(matches)); for _, m := range matches { out = append(out, m.cmd); if len(out) == 8 { break } }; return out
}
