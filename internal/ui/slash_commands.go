...
type SlashCommand struct {
	...
	TextArg     bool     // whether the argument is required free-form text rather than a positive integer
...
// ParseSlashCommand parses a slash command line.
...
func FilterSlashCommands(query string) []SlashCommand {
	...
	for _, cmd := range supportedSlashCommands {
		...
		if rank <= 4 { matches = append(matches, match{cmd: cmd, rank: rank}) }
	}
	sort.Slice(matches, func(i, j int) bool {
		if matches[i].rank != matches[j].rank { return matches[i].rank < matches[j].rank }
