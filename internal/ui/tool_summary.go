...
func toolSummaryMetadata(t *presentation.ToolCard) []string {
	...
	if t.Duration > 0 {
		out = append(out, dur(t.Duration))
	}
	if t.Truncated {
		out = append(out, "truncated")
	}
	if t.OutputState == presentation.OutputTruncated {
		out = append(out, "saved output truncated")
	}
	if t.OutputState == presentation.OutputUnavailable {
		out = append(out, "output unavailable")
	}
	if t.NextOffset != nil {
		out = append(out, fmt.Sprintf("next offset %d", *t.NextOffset))
	}
	return out
}
...
func toolImportantMetadata(t *presentation.ToolCard) string {
	...
	if t.Signal != "" {
		return toolSummaryText(t.Signal)
	}
	if t.Status == presentation.ToolFailed {
		return "failed"
	}
	return ""
}
...
func renderToolMetadata(t *presentation.ToolCard, width int) []string {
	...
	if t.ExitCode != 0 {
		add("exit", fmt.Sprint(t.ExitCode))
	}
	add("signal", t.Signal)
	add("error", t.Err)
	if t.Truncated {
		add("read", "truncated at execution")
	}
	if t.OutputState == presentation.OutputTruncated {
		add("output", "saved prefix only")
	}
	if t.OutputState == presentation.OutputUnavailable {
		add("output", "unavailable")
	}
	if t.NextOffset != nil {
		add("next", fmt.Sprintf("offset=%d", *t.NextOffset))
	}
	return rows
}
