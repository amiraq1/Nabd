...
func (m *Feed) navigationItems() []presentation.FeedItem {
	...
	if len(items) > maxVisibleFeedItems { items = items[len(items)-maxVisibleFeedItems:] }
	return items
}

func (m *Feed) selectItem(index int) {
	items := m.navigationItems()
	if len(items) == 0 { m.selectedItem = -1; return }
	if index < 0 { index = 0 }
	if index >= len(items) { index = len(items)-1 }
	m.selectedItem = index
	_, offsets := renderItemsWithOffsets(items, m.width, m.toolsExpanded)
	if index < len(offsets) { m.follow = false; m.scrollTop = offsets[index]; m.clampScroll() }
}
...
func (m *Feed) selectType(kind presentation.ItemType, forward bool) {
	...
	for i := 1; i <= len(items); i++ {
		idx := (start+i*step)%len(items); if idx < 0 { idx += len(items) }
...
func (m *Feed) navigationKey(k tea.KeyMsg) (tea.Model, tea.Cmd, bool) {
	switch k.Type {
	case tea.KeyUp: m.moveCard(-1); return m,nil,true
	case tea.KeyDown: m.moveCard(1); return m,nil,true
	case tea.KeyEnter:
		items := m.navigationItems()
		if m.selectedItem >= 0 && m.selectedItem < len(items) && items[m.selectedItem].Type == presentation.ItemTool { model,cmd := m.toggleTools(); return model,cmd,true }
		return m,nil,true
	case tea.KeyEsc: m.navigationMode=false; m.status=""; m.composer.focus(); return m,nil,true
	}
	...
	case "?": if m.status=="" { m.status=navigationHint(m.width) } else { m.status="" }; return m,nil,true
	}
	return m,nil,false
}
