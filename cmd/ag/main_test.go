...
// TestSystemModelDirection: the system prompt is model-facing text sent with
// every request. It must be English words (model-directed strings are English
// only) AND carry an explicit output-language directive — otherwise the Arabic
// replies are lost, which is exactly the value the translation keeps.
//
// The prompt itself now lives in internal/payload (NBD-403), because its size
// is part of the fixed per-request payload that package budgets.
func TestSystemModelDirection(t *testing.T) {
	for _, r := range payload.DefaultSystemPrompt {
	...
	for _, want := range []string{
		...
		if !strings.Contains(payload.DefaultSystemPrompt, want) {
